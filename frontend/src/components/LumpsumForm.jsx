...
export default function LumpsumForm({ onResult }) {
  const [form, setForm]       = useState({ investment_amount: "", annual_return_rate: "", duration_years: "" });
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = await api.lumpsum(form);
      onResult(data);
  ...
  return (
    <form onSubmit={submit} style={formStyle}>
      <h2 style={heading}>💰 Lumpsum Investment</h2>
      <Field label="Investment Amount (₹)" name="investment_amount"  value={form.investment_amount}  onChange={handle} placeholder="e.g. 100000" />
