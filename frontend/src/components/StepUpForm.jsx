  ...
  return (
    <form onSubmit={submit} style={formStyle}>
      <div style={headerStyle}>
        ...
        <h2 style={headingStyle}>Step-Up SIP Calculator</h2>
        <p style={subStyle}>Increase your SIP amount every year automatically.</p>
      </div>
      ...
      <Field label="Annual Step-Up (%)"         name="annual_step_up_percent" value={form.annual_step_up_percent} onChange={handle} placeholder="e.g. 10" />
      {error && <p style={errorStyle}>⚠️ {error}</p>}
      <button type="submit" style={btnStyle} disabled={loading}>
        {loading ? "⏳ Calculating..." : "Calculate Step-Up SIP →"}
      </button>
      <p style={noteStyle}>SIP increases by step-up % every year</p>
    </form>
  );
}
